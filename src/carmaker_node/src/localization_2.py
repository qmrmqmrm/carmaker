...
class Localization():
	...
	def subscribe(self):
		self.localization_pub = rospy.Publisher('/localization_2', localization_2, queue_size=10)
		...
		ts = message_filters.ApproximateTimeSynchronizer([self.gps_data,self.imu_data,self.udp_data], 2, 0.1, allow_headerless=True)
	...
	def callback(self,gps,imu      ,udp):
		...
		self.val_set(imu, udp)				# value set
	...
	def KF(self):
		...
		if self.start_pose_flag is False :		
			self.x_old = self.start_x
			self.y_old = self.start_y
			self.theta_old = self.Theta
			self.start_pose_flag = True

		x_new, y_new, theta_new = self.calc_odometry(self.x_old,self.y_old,self.theta_old,self.v,self.w)
		self.result_x = x_new
		self.result_y = y_new
